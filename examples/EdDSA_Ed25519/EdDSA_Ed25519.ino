...
void loop()
{
    ...
    returnVal = tropic01.eccKeyRead(ECC_SLOT_ED25519, ed25519PubKey, sizeof(ed25519PubKey), curveType,
                                    keyOrigin);
    if (returnVal != LT_OK) {
        printLibtropicError("  Tropic01.eccKeyRead() failed, returnVal=", returnVal);
        returnVal = tropic01.eccKeyErase(ECC_SLOT_ED25519);
        if (returnVal != LT_OK) {
            printLibtropicError("  Additionally, failed to erase the Ed25519 key slot, returnVal=",
                                returnVal);
        }
        cleanResourcesAndLoopForever();
    }
    ...
    returnVal = tropic01.eddsaSign(ECC_SLOT_ED25519, (const uint8_t *)message, messageLen,
                                   ed25519Signature);
    if (returnVal != LT_OK) {
