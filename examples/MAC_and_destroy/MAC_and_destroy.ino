 ...
 * @brief TROPIC01 MAC-and-Destroy PIN verification example using the C++ wrapper for libtropic.
 ...
 * 1. Uses slots in TROPIC01's flash memory (up to 128 slots, 0-127)
 ...
 * - Non-volatile storage in R memory
 *
...
// Valid range: 1 to 128 (TR01_MACANDD_ROUNDS_MAX)
// Adjust this value to set the number of allowed PIN attempts.
...
// MAcand-Destroy needs non volatile storage for storing part of the date, which are used during the process of PIN
// verification. In this example we are using last slot in TROPIC01 r-memory area for this.
...
struct MacAndDestroyNVM {
    ...
    uint8_t encryptedSecrets[MACANDD_ROUNDS * 32];  // Encrypted master secrets (c_i)
...
static void printHex(const char *label, const uint8_t *data, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (i < len - 1 && (i + 1) % 32 != 0) Serial.print(" ");
        if ((i + 1) % 32 == 0 && (i + 1) < len) {
            Serial.println();
            Serial.print("  ");
        }
...
static void xorCrypt(const uint8_t *input, const uint8_t *key, uint8_t *output, size_t len)
{
    for (size_t i = 0; i < len; i++) {
...
static psa_status_t hmacSha256(const uint8_t *key, size_t keyLen, const uint8_t *data, size_t dataLen, uint8_t *output)
{
 ...
 */
static lt_ret_t pinSetup(const uint8_t *masterSecret, const uint8_t *pin, uint8_t pinSize, uint8_t *finalKey)
{
    ...
    for (int i = 0; i < MACANDD_ROUNDS; i++) {
        ...
        ret = tropic01.macAndDestroy(i, u, ignore);
        if (ret != LT_OK) {
            Serial.print("    Slot ");
            Serial.print(i);
        ...
        ret = tropic01.macAndDestroy(i, v, w_i);
        if (ret != LT_OK) {
            return ret;
        }
    ...
    if (hmacSha256(masterSecret, 32, (uint8_t *)"2", 1, finalKey) != PSA_SUCCESS) {
        return LT_FAIL;
    }
 ...
 */
static lt_ret_t pinVerify(const uint8_t *pin, uint8_t pinSize, uint8_t *finalKey)
{
    ...
    ret = tropic01.macAndDestroy(nvm.attemptCount, v_, w_i);
    if (ret != LT_OK) {
        Serial.print("    Failed, returnVal=");
        Serial.println(ret);
        return ret;
    ...
    for (int x = nvm.attemptCount; x < MACANDD_ROUNDS - 1; x++) {
        ret = tropic01.macAndDestroy(x, u, ignore);
        if (ret != LT_OK) {
            return ret;
        }
    }
...
void setup()
{
    ...
    // ==================== PIN Setup ====================
    Serial.println("==================== Step 1: PIN Setup ========================");
    Serial.println();

    // Generate random master secret.
    uint8_t masterSecret[32];
    Serial.println("Generating random master secret...");
    // In production, use TRNG. For demo, use pseudo-random.
    for (int i = 0; i < 32; i++) {
        masterSecret[i] = random(256);
    }
    ...
    for (int i = 0; i < sizeof(pin); i++) {
        Serial.print(pin[i]);
        if (i < sizeof(pin) - 1) Serial.print(", ");
    }
    Serial.println();
    Serial.println();
    ...
    printHex("Final Key (from setup)", finalKeySetup, 32);

    Serial.println("===============================================================");
    Serial.println();

    // ==================== PIN Verification - Wrong Attempts ====================
    Serial.println("=========== Step 2: Testing Wrong PIN Attempts ================");
    Serial.println();
    ...
    for (int i = 0; i < sizeof(wrongPin); i++) {
        ...
        if (i < sizeof(wrongPin) - 1) Serial.print(", ");
    }
    Serial.println();
    Serial.println();
    ...
    Serial.println("========== Step 3: Correct PIN Verification ===================");
    Serial.println();
    ...
    printHex("Final Key (from verify)", finalKeyVerify, 32);
    Serial.println();
    ...
    Serial.println("===============================================================");
    Serial.println();
    Serial.println("============ MAC-and-Destroy Test Complete! ===================");
    Serial.println("===============================================================");
    Serial.println();
    Serial.println("Summary:");
    Serial.print("  - PIN attempts configured: ");
    Serial.println(MACANDD_ROUNDS);
    Serial.print("  - Wrong attempts tested: ");
    Serial.println(wrongAttempts);
    Serial.println("  - Correct PIN verification: SUCCESS");
    Serial.println("  - Final key match: SUCCESS");
    Serial.println();
    Serial.println("The PIN verification system is working correctly!");
    Serial.println("Slots have been reinitialized and are ready for next use.");
    Serial.println("===============================================================");
}
// -----------------------------------------------------------------------------------------------------
