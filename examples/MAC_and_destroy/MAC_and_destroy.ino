 ...
 * - Non-volatile storage in TROPIC01's R-Memory - adapt to your platform.
 *
...
static lt_ret_t pinSetup(const uint8_t *masterSecret, const uint8_t *pin, const uint8_t pinSize, uint8_t *finalKey)
{
    ...
    // memset() is used here for simplicity; consider using a more secure method in your application.
    memset(v, 0, sizeof(v));
...
static lt_ret_t pinVerify(const uint8_t *pin, const uint8_t pinSize, uint8_t *finalKey)
{
    ...
    if (memcmp(nvm.tag, t_, sizeof(nvm.tag)) != 0) {
...
void loop()
{
    ...
    printHex("Master Secret", myMasterSecret, MY_MASTER_SECRET_SIZE);
    Serial.println();

    Serial.print("PIN: ");
    for (int i = 0; i < sizeof(myPin); i++) {
    ...
    if (memcmp(finalKeySetup, finalKeyVerify, sizeof(finalKeySetup)) == 0) {
