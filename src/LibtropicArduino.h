...
class Tropic01 {
   public:
     ...
     * @param[in] intGpioPin   GPIO pin where TROPIC01's interrupt pin is connected (is one of the
     ...
     * `<SPI.h>`
     * @param[in] spiSettings  SPI settings, defaults to tested values. If you want to change them,
     * keep `SPISettings.dataOrder=MSBFIRST` and `SPISettings.dataMode=SPI_MODE0` (required by
     * TROPIC01).
     */
     ...
     * @warning SPI.begin() is not called by this method, user must ensure SPI is initialized before
     * calling this method.
     *
     ...
     * @warning SPI.end() is not called by this method, user must ensure SPI is deinitialized after
     * calling this method.
     *
     * @retval  LT_OK  Method executed successfully
     * @retval  other  Method did not execute successfully, you might use lt_ret_verbose() to get
     ...
     *
     * @retval                LT_OK Method executed successfully
     * @retval                other Method did not execute successfully, you might use lt_ret_verbose()
     * to get verbose encoding of returned value
     */
    lt_ret_t secureSessionStart(const uint8_t shiPriv[], const uint8_t shiPub[],
    ...
    /**
     * @brief Executes the TROPIC01's Ping command. It is a dummy command to check the Secure Channel
     * Session is valid by exchanging a message with TROPIC01, which is echoed through the Secure
     * Channel.
     *
     ...
     * @retval             other Function did not execute successfully, you might use lt_ret_verbose()
     * to get verbose encoding
     */
     ...
     *
     * @retval           LT_OK Method executed successfully
     * @retval           other Method did not execute successfully, you might use lt_ret_verbose() to
     ...
     * get verbose encoding of returned value
     */
    lt_ret_t eccKeyStore(const lt_ecc_slot_t slot, const lt_ecc_curve_type_t curve,
                         const uint8_t key[]);
     ...
     * @param curve[out]        Type of elliptic curve public key (TR01_CURVE_ED25519 or
     * TR01_CURVE_P256)
     * @param origin[out]       Origin of the public key (TR01_KEY_GENERATED or TR01_KEY_STORED)
     *
     * @retval                  LT_OK Method executed successfully
     * @retval                  other Method did not execute successfully, you might use
     * lt_ret_verbose() to get verbose encoding of returned value
     */
    lt_ret_t eccKeyRead(const lt_ecc_slot_t slot, uint8_t key[], const uint8_t keyMaxSize,
     ...
     * get verbose encoding of returned value
     */
    lt_ret_t ecdsaSign(const lt_ecc_slot_t slot, const uint8_t msg[], const uint32_t msgLen,
                       uint8_t rs[]);
     ...
     * get verbose encoding of returned value
     */
    lt_ret_t eddsaSign(const lt_ecc_slot_t slot, const uint8_t msg[], const uint16_t msgLen,
                       uint8_t rs[]);
     ...
     * @param dataSize[in]   Size of data to be written into slot. Minimal size is
     * TR01_R_MEM_DATA_SIZE_MIN, maximal size depends on TROPIC01 Application FW and is either 444B
     * (TROPIC01 App FW version <2.0.0) or 475B (TROPIC01 App FW version >=2.0.0)
     *
     * @retval               LT_OK Method executed successfully
     * @retval               other Method did not execute successfully, you might use lt_ret_verbose()
     * to get verbose encoding of returned value
     ...
     * lt_ret_verbose() to get verbose encoding of returned value
     */
    lt_ret_t rMemRead(const uint16_t udataSlot, uint8_t data[], const uint16_t dataMaxSize,
     ...
     *
     * @param slot[in]      MAC-and-Destroy slot index (TR01_MAC_AND_DESTROY_SLOT_0 -
     ...
     * to get verbose encoding of returned value
     */
    lt_ret_t macAndDestroy(const lt_mac_and_destroy_slot_t slot, const uint8_t dataOut[],
                           uint8_t dataIn[]);

    /**
     * @brief Reads the unique chip ID from TROPIC01.
     *
     * @param chipId[out]  Structure to be filled with the chip ID read from TROPIC01
     *
     * @retval             LT_OK Method executed successfully
     * @retval             other Method did not execute successfully, you might use lt_ret_verbose() to
     * get verbose encoding of returned value
    ...
    /**
     * @brief Prints the contents of a chip ID structure using the provided print function.
     *
     * @param chip_id[in]     Chip ID structure to be printed (e.g. obtained from getChipID)
     * @param print_func[in]  Pointer to a printf-style function used to output the formatted text
     * (e.g. printf)
     *
     * @retval                LT_OK Method executed successfully
     * @retval                other Method did not execute successfully, you might use lt_ret_verbose()
     * to get verbose encoding of returned value
     */
    lt_ret_t printChipID(const lt_chip_id_t &chip_id, int (*print_func)(const char *format, ...));

    /**
     * @brief Reads the bootloader firmware version from TROPIC01.
     * @details The chip is temporarily rebooted into maintenance mode to read the bootloader version
     * and then rebooted back to its original mode.
     *
     * @param fw_ver[out]  Buffer for the firmware version bytes (must be
     * TR01_L2_GET_INFO_RISCV_FW_SIZE bytes, i.e. 4 bytes)
     *
     * @retval             LT_OK Method executed successfully
     * @retval             other Method did not execute successfully, you might use lt_ret_verbose() to
     * get verbose encoding of returned value
    ...
    /**
     * @brief Prints the firmware headers (FW1, FW2, SPECT1, SPECT2) stored in TROPIC01 using the
     * provided print function.
     * @details The chip is temporarily rebooted into maintenance mode to read the firmware headers and
     * then rebooted back to its original mode.
     *
     * @param print_func[in]  Pointer to a printf-style function used to output the formatted text
     * (e.g. printf)
     *
     ...
     * @details If the chip is in maintenance mode, it is temporarily rebooted into application mode to
     * read the version and then rebooted back to maintenance mode.
     *
     * @param fw_ver[out]  Buffer for the firmware version bytes (must be
     * TR01_L2_GET_INFO_RISCV_FW_SIZE bytes, i.e. 4 bytes)
     *
    ...
    /**
     * @brief Reads the SPECT firmware version from TROPIC01.
     * @details If the chip is in maintenance mode, it is temporarily rebooted into application mode to
     * read the version and then rebooted back to maintenance mode.
     *
     * @param fw_ver[out]  Buffer for the firmware version bytes (must be
     * TR01_L2_GET_INFO_SPECT_FW_SIZE bytes, i.e. 4 bytes)
     *
     * @retval             LT_OK Method executed successfully
     * @retval             other Method did not execute successfully, you might use lt_ret_verbose() to
     * get verbose encoding of returned value
    ...
    /**
     * @brief Gets random bytes generated by TROPIC01.
     *
     * @param rand_buf[out]  Buffer to store the generated random bytes
     * @param rand_len[in]   Number of random bytes to generate (max TR01_RANDOM_VALUE_GET_LEN_MAX,
     * i.e. 255)
     *
     * @retval               LT_OK Method executed successfully
     * @retval               other Method did not execute successfully, you might use lt_ret_verbose()
     ...
     *
     * @param index[in]  Monotonic counter index (TR01_MCOUNTER_INDEX_0 - TR01_MCOUNTER_INDEX_15)
     * @param value[in]  Initial value of the counter (allowed range is 0 - TR01_MCOUNTER_VALUE_MAX)
     *
     * @retval           LT_OK Method executed successfully
     * @retval           other Method did not execute successfully, you might use lt_ret_verbose() to
     * get verbose encoding of returned value
    ...
    /**
     * @brief Gets the current value of the monotonic counter of a given index.
     *
     * @param index[in]   Monotonic counter index (TR01_MCOUNTER_INDEX_0 - TR01_MCOUNTER_INDEX_15)
     * @param value[out]  Current value of the monotonic counter (from range 0 -
     * TR01_MCOUNTER_VALUE_MAX)
     *
     * @retval            LT_OK Method executed successfully
     * @retval            other Method did not execute successfully, you might use lt_ret_verbose() to
     * get verbose encoding of returned value
    ...
    /**
     * @brief Updates (decrements by one) the monotonic counter of a given index.
     *
     * @param index[in]  Monotonic counter index (TR01_MCOUNTER_INDEX_0 - TR01_MCOUNTER_INDEX_15)
     *
     * @retval           LT_OK Method executed successfully
